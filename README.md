# 🏆 比賽管理系統 (Competition Manager) v2.25.0
...
### v2.25.0 (2026-09-26) - 推播時間與事件可設定（＋修好頁面上方的版本號）

- **為什麼**：推播一直是「系統說了算」——每天固定 09:00（澳門時間）發一次、只有「即將開賽」與「新發布賽事」，管理員想改時間或暫時關掉某一類通知只能改程式；另外頁面上方「發佈與管理各類賽事資訊 **(v2.18.0)**」從 v2.18.0 之後就沒再更新（寫死在 HTML 裡）。這一版兩件事一起處理。
- **⏰ 推播設定（管理員以上，導覽選單）**：每日摘要總開關／發送時間（澳門時間）／摘要要包含哪些事件（新發布賽事、即將開賽提醒）／即時通知的事件（報名審核結果、候補遞補通知）可各自開關。儲存後畫面直接回報**每一項的前後變化**並回讀確認；留稽核（新動作：更新推播設定，記前後值）。設定存在既有 `app_settings` 鍵值表，**不需要 migration**。
- **一天只發一次、時間到就補發**：伺服器每天檢查兩次（Vercel cron 01:00／13:00 UTC＝澳門 09:00／21:00），**只有「當地日期還沒發過且已過設定時間」才發**，並記下最後發送日期；設定的時間若在兩次檢查之間（例如 18:00），當天 21:00 那次會補發。沒訂閱者／沒到期賽事／事件全關掉都會明確回報原因，不是靜悄悄。
- **關掉的事件是真的關掉**：被關掉的即時通知**連推播訂閱都不會查**（不是查了才不送），審核、遞補、稽核流程照常；讀不到設定時**一律視為開啟**（情願多發不漏發）並在面板提示降級；推播這一段失敗也不讓排程倒下（稽核清理與週期性賽事照做，原因回報在 `digest.reason`）。
- **版本號不再忘記改**：頁面上方副標改由 `GET /api/version` 帶入（單一來源＝`package.json`），HTML 不再寫死；新增守門測試 `tests/version-consistency.test.js`（package.json＝`<title>`、頁面不得再出現寫死的 `(vX.Y.Z)`、副標必須由 `/api/version` 更新、`/api/version` 要等於 package.json）。
- **驗證**：`npm test` **360 項全綠**（新增推播設定純函式 21＋推播設定 API 14＋未執行 migration 降級 3＋版本一致性 4）；`npm run check:browser` **322 項全綠**（新增 `push-settings-check.js` **39 項**：真 Chrome 手機尺寸實際開面板、改時間與事件、儲存後重開確認、壞時間被擋下且不污染資料庫、關掉審核通知後審核一筆報名「連訂閱都不查」、關掉摘要後排程明確回報，並確認頁面上方版本號正確、沒有前端例外、沒有下載任何檔案）。

### v2.24.0 (2026-09-26) - 賽事複製與週期性賽事

- **為什麼**：開一場新賽事要把上一場的地點、時間、分類、隊伍設定、報名窗、審核與候補規則再填一次；每週三的例賽、每兩週的積分賽、每月的排名賽更是內容幾乎一樣、只有日期往後推。
- **一鍵複製賽事**：每張卡片（管理員）新增「📄 複製賽事」。**照抄設定**（地點／日期／時間／說明／分類與標籤／隊伍設定／報名窗／需審核／開放候補／遞補通知開關），**不搬進度**（報名紀錄、隊伍、海報）；名稱加「（複製）」且不會變成「（複製）（複製）」；複製品**不帶週期**（是一次性的複製品，不會被排程當成系列成員）；留稽核（複製賽事）。
- **週期性賽事**：卡片「🔁 週期」設定每週／每兩週／每月與「重複到哪一天」（留空＝一直重複），設定後按鈕直接顯示目前週期（例如「🔁 每兩週」）。建立下一場有兩個管道、**同一份判斷規則**：每日排程（現有的 `/api/cron/reminders`，不用另外設定 cron）與面板上的「⬇️ 立即建立下一場」。規則：下一場＝系列最後一場往後推一個週期（**每月遇缺日夾到月底**，1/31 → 2/28 不跳月）、**只提前 30 天內**建立、**一次只排一場**（系列還有未來的場次就不會再疊）、**報名時間一起往後挪**同樣天數、超過結束日就停、週期落後時不偷偷補而是回報「已落後」。週期是整個系列的設定，改一場會同步其他場次（介面回報同步幾場）。稽核：設定賽事週期／週期性賽事建立下一場（排程觸發時操作者記 `system`）。
- **migration**：`2026-09-26-v2.24.0-recurrence.sql`（`recurrence`／`recurrence_until`／`recurrence_parent_id`＋部分索引，可重複執行）。**未執行時**：複製照常、週期端點回 503 指引檔名、每日排程不會壞（週期部分回報未啟用）。
- **驗證**：`npm test` **318 項全綠**（新增週期規則 21＋複製與週期 API 16＋未執行 migration 降級 4）；`npm run check:browser` **283 項全綠**（新增 `recurrence-check.js` **34 項**：手機尺寸真 Chrome 複製一場、設定每週、建立下一場、已排定時不重複建立、取消週期，並確認沒有前端例外、沒有下載任何檔案）。
- **邊界都有測**：月底夾日期、閏年、跨年、結束日剛好是下一場那天、系列已排定未來場次、重複按「建立下一場」。

### v2.23.0 (2026-09-26) - 候補收尾：多選一次搬多筆、異動紀錄可以往回翻

- **為什麼**：v2.22.0 一次只能搬一筆（搬第一筆就會讓其他人的位置位移，管理員得邊搬邊重算）；異動紀錄只看得到最近 30 筆，賽事中後期查不到前幾週「為什麼這個人排在那裡」。
- **多選一次搬多筆**：候補每列加勾選框，勾選後出現批次列（「已選 N 筆」＋「搬到 第 N 位」＋搬過去／清除選取）。**搬完的相對順序＝名單上原本的相對順序**（不是勾選先後）；一次搬 N 筆時目標位置只到「人數 − N + 1」；搬完勾選自動清空、批次列收起；名單期間被別人改動時由後端整批拒絕（沿用 v2.21.0 的完整順序驗證，**不需要新端點、不需要新 migration**）。位置換算走共用純函式 `CMCompetitionState.moveGroup()`。
- **異動紀錄往回翻**：`GET /api/competitions/:id/waitlist/history?limit=30&offset=30` 新增 `offset`／`has_more`／`capped`；面板加「⬇️ 載入更早」與說明（「已顯示 30 筆」→「已顯示全部 43 筆」，按鈕自動收起；上限 500 筆時提示改用稽核日誌頁）。抓取窗口＝本頁結尾 + 1，**分頁不重複也不漏**（測試有驗）。
- **檢查腳本不再下載檔案到使用者的電腦**：`tests/browser/lib/cdp.js` 啟動時用 CDP 設定「拒絕下載」，另提供 `guardDownloads()` 在頁面層攔下 `<a download>`；匯出類檢查改用假 blob URL，只攔內容來驗（會斷言「下載被攔下來」並顯示檔名）。
- **驗證**：`npm test` **277 項全綠**（新增批次搬移規則 7＋異動紀錄分頁 4）；`npm run check:browser` **249 項全綠**（新增 `waitlist-batch-check.js` **23 項**：批次列行為、40 筆種子紀錄的兩頁分頁不重複、批次搬移後畫面與資料庫同步、勾選自動清空）。

### v2.22.0 (2026-09-26) - 候補再補強：拖拉排序、跳到第 N 位、順位異動紀錄、匯出補欄位

- **為什麼**：v2.21.0 只能按 ↑／↓ 一位一位搬（從第 8 位搬到第 2 位要按 6 次、每次都等一次 API）；調完之後看不出「誰在什麼時候調了什麼」；匯出的報名名單看不出狀態與候補順位。
- **拖拉排序**：候補每一列左側加拖曳把手 **⠿**，用 **pointer 事件**（桌機滑鼠與手機觸控同一條路徑、不用外部套件、不放寬 CSP）：拖動時只動畫面、**放開才送一次完整順序**，失敗會把畫面拉回真實狀態。把手有 `touch-action: none`（沒寫的話手機拖不動，只會捲動頁面）。
- **「第 N 位」下拉**：每一列可一次搬到任意順位（取代連按 ↑／↓），選到原位不送 API；也是**鍵盤使用者**唯一能遠距搬移的方式。
- **「🕘 順位異動」面板**：新端點 `GET /api/competitions/:id/waitlist/history?limit=30`（管理員，上限 100），**直接讀稽核日誌**（不另開表，紀錄與稽核永遠同一份）：調整順位、手動／指定遞補、取消自動遞補、審核、通知設定、取消報名，每筆有時間、中文動作名稱、操作者與內容。
- **每賽事的「遞補通知」開關**：新端點 `POST /api/competitions/:id/waitlist/notify`。**關掉的是通知，不是遞補**——狀態、名單、稽核照常，只是**完全不碰推播（連訂閱都不查）**；三條遞補路徑（指定／下一位／取消自動遞補）都遵守；回報與稽核寫明「依賽事設定未通知」。migration `2026-09-26-v2.22.0-waitlist-notify.sql`；未執行時**一律視為通知開啟**（既有行為不變），介面把開關變停用並說明。
- **匯出報名名單 CSV 補「狀態」「候補順位」**：現場報到直接看得懂誰已核准、誰是候補第幾位。
- **順手修掉一個老 bug**：`CMCSV.stringify()` 只認陣列列，傳物件列會產生「下載成功、打開全是空行」的 CSV——報名名單匯出其實一直是壞的。現在物件列會自動產生表頭＋資料，並補上單元測試與「攔下載驗內容」的瀏覽器檢查（就是這個檢查抓到的）。
- **驗證**：`npm test` **266 項全綠**（新增位置搬移 7＋通知開關 API 8＋異動紀錄 API 7＋未執行 migration 4＋CSV 物件列 3）；`npm run check:browser` **225 項全綠**（新增 `waitlist-extra-check.js` **29 項**，含合成 pointer 事件的真實拖曳、通知開關、異動面板、CSV 內容驗證）。

### v2.21.0 (2026-09-26) - 候補順位的兩個實務缺口：指定遞補、手動調整順序

- **問題**：v2.20.0 的自動遞補只補第一位。但候補第一名可能聯絡不到、或根本不是需要的組別——舊做法只能先「取消他的報名」再讓系統補下一位，那是破壞性動作（對方收到通知、紀錄也變了）。另外需要把某些人往前排時，v2.20.0 唯一的辦法是改報名時間，等於偽造資料。
- **本版做了什麼**：
  - **指定遞補（不照順位）**：`POST /api/registrations/:id/promote`，管理員可直接挑候補名單裡任何一位遞補，**被跳過的人完全不受影響**。名額守門與「遞補下一位」共用同一份純函式（名額已滿 400、不在候補名單 400／404），需審核的賽事遞補後仍是待審核；留稽核（內容註明「未照順位」）與推播通知，`review_note` 記「指定遞補（原第 N 順位）」。
  - **候補順位手動調整**：`POST /api/competitions/:id/waitlist/reorder`，送出完整順序一次寫入 `waitlist_order`；**漏一筆／多一筆／重複一律整批拒絕且不改任何資料**。排序規則（前後端共用）：兩筆都手動排過照數字、只有一筆排過的在前面、都沒排過照報名時間 → **新報名的人一律排到最後**，沒被動到的人維持原相對順序。
  - **介面**：候補每列加 `↑`／`↓`（與隔壁對調，頭尾自動停用）與「⬆️ 遞補」（先確認會跳過前面的人）；區塊標題下方顯示順位來源（依報名時間／管理員手動排定）；**名額已滿時每列的遞補按鈕停用**；「我的報名」的候補順位跟著管理員排定的順序走。
  - **migration**：`migrations/2026-09-26-v2.21.0-waitlist-order.sql`（`waitlist_order` 欄位＋索引，可重複執行）。**未執行時分兩級降級**：調整順位回 503＋指引且不寫入任何資料、順位自動退回「先報名先排」（舊行為不變）、**指定遞補照常可用**（不需要新欄位）、介面不顯示 ↑／↓ 並在說明文字講清楚。
  - 順手把「遞補下一位」的名額守門改用共用的 `planPromotion()`，兩條遞補路徑不可能再走鐘。
- **驗證**：`npm test` **237 項全綠**（新增排序規則 15 項＋API 10 項＋未執行 migration 4 項）；`npm run check:browser` **196 項全綠**（新增 `waitlist-manage-check.js` **32 項**：畫面與資料庫順位同步、關掉再開順序仍在、指定遞補跳過第一位、名額滿時按鈕停用與硬打被擋、一般用戶 403、當事人的我的報名順位同步）。

### v2.20.0 (2026-09-26) - 報名審核與候補名單（額滿自動進候補、有人取消自動遞補並通知）

- **問題**：比賽常常需要「先審核再算報名成功」、名額滿了要收候補、有人取消要通知下一位——舊版一個都做不到（額滿只能吃閉門羹，候補得自己在 LINE 記，取消後空出的名額常常忘了補）。
- **名額規則（寫在同一份共用模組）**：**佔名額＝已核准（`confirmed`）＋待審核（`pending`）**，候補（`waitlisted`）不佔名額。賽事有兩個開關：**報名需審核**（`requires_approval`）、**額滿開放候補**（`waitlist_enabled`），四種組合都支援。
- **本版做了什麼**：
  - **報名審核**：管理員可在報名／隊伍視窗逐筆「✅ 核准」「❌ 拒絕」（拒絕可寫原因），兩種結果都會推播通知報名者並寫稽核（`REGISTER_APPROVED`／`REGISTER_REJECTED`）。**核准時名額已滿會擋（400）**，不能靠審核超收。
  - **候補名單**：額滿且開放候補時仍可報名，會排進候補並回報**第幾順位**；卡片顯示「🕒 額滿，報名排候補（已有 N 人候補）」，報名按鈕變「📝 報名（排候補）」，送出前先說明會排到第幾位。
  - **自動遞補**：名額讓出來（有人取消報名、或管理員取消／改拒絕）→ **第一位候補自動遞補**，立即推播通知（`push_log` 記 `waitlist_promoted`）並寫稽核（`AUTO_PROMOTE_WAITLIST`）。需審核的賽事遞補後仍是「待審核」，不會因為遞補繞過審核。另有管理員手動「⬆️ 遞補下一位」（`PROMOTE_WAITLIST`；名額已滿時停用並註明原因）。
  - **人數統計改以「佔名額」為準**：報名卡片與 `/api/registration-counts` 只算已核准＋待審核，另外回傳每場候補人數。
  - **未編排報名者只列已核准的人**：待審核或候補的人不能被編進隊伍（後端也會擋），避免審核前先佔位。
  - **我的報名顯示狀態**：待審核／已核准／候補 第 N 位／未錄取（含拒絕原因）；未錄取與被取消的報名不再顯示取消按鈕。
  - **資料庫欄位**：`migrations/2026-09-26-v2.20.0-registration-review.sql`（全部 `IF NOT EXISTS`，可重複執行）。**未執行時功能整組優雅停用**（報名照舊直接核准、審核／遞補端點回 503 並附指引、表單不寫入不存在的欄位、介面誠實說明）。
- **順手修掉**：`regWaitlistCounts` 漏宣告（會讓整個賽事列表顯示「無法載入比賽資料」，`node --check` 抓不到、真實瀏覽器檢查才抓到）；`setTeamMsg` 參數順序寫反（介面只顯示「success」）。
- **驗證**：`npm test` **208 項全綠**（新增審核／候補規則 11 項＋API 14 項＋未執行 migration 6 項）；`npm run check:browser` **164 項全綠**（新增 `registration-review-check.js` **49 項**，真實 Chrome 手機尺寸，從使用者與管理員兩個視角走完審核、候補、自動遞補、表單開關與權限）。
...
- **問題**：v2.14.0 的巡檢只會「讀」線上錯誤日誌並產生待修復清單，讀完沒有收尾——清單會一直累積同一批舊訊息，首頁提示橫幅也永遠亮著，新的問題反而被舊雜訊蓋掉。
- **本版做了什麼**：
  - 新增 `POST /api/admin/error-logs/resolve`：**批次**標記已處理。只接受明確的 id 清單（單次最多 500 筆）或 `all_unresolved: true`（內部上限 1000 筆），一定留 `RESOLVE_ERROR_LOGS` 稽核（含筆數與前 20 個 id），未執行 v2.12.0 migration 時回 503 並附檔案路徑。
  - `npm run triage`（發版時順便跑的巡檢）現在**讀取 → 分析 → 把剛讀到的那批未處理紀錄標記為已處理**，`resolved_by` 記為巡檢身分。想只看不動資料可用 `npm run triage:report`（或 `--no-resolve`）。
  - 第九節（待修復清單）會記錄「本輪已標記 N 筆」，報告本身仍是那批紀錄的永久存檔——**標記不等於刪除**，取消勾選「只看未處理」仍可查看全部。
  - 錯誤日誌視窗新增「🏷️ 全部標記已處理」按鈕（走同一個端點），按下前有明確確認、完成後橫幅與選單計數立即更新。
  - 標記失敗不會中斷巡檢：報告照常產出，下次巡檢會再試（並在輸出留下警告）。
- **驗證**：`npm test` 139 項全綠（新增批次標記 API 7 項、migration 缺失降級 1 項、巡檢標記純函式與呼叫 4 項）；`npm run check:browser` 新增「標記已處理」介面檢查 13 項（真實 Chrome 手機尺寸：橫幅出現 → 3 筆未處理 → 一鍵標記 → 清單變已處理且紀錄還在 → 橫幅與計數消失 → 稽核留下紀錄）。

### v2.16.0 (2026-09-26) - 稽核日誌強化（篩選、分頁、匯出、保留天數）

- **問題**：稽核紀錄一直有在寫，但以前只能看最近 100 筆、不能篩選也不能匯出；出事後要追「某人某段時間做了什麼」幾乎不可能。
- **本版做了什麼**：
  - 可依 **使用者／動作／時間區間／關鍵字** 篩選，關鍵字比對使用者、動作、目標與詳情內容（大小寫不敏感）。
  - **分頁瀏覽**（每頁 100 筆，可翻新舊頁），並顯示目前範圍與總數（拿不到精確總數時老實顯示「可能還有更多」，不假造數字）。
  - **匯出 CSV**：套用同一組篩選條件、含中文動作說明欄、帶 BOM（Excel 開中文不亂碼）、沿用試算表公式注入防護。
  - **保留天數清理**：可先「預覽會刪幾筆」再確認；保留天數下限 30 天（避免手誤刪光）；**預設不會自動刪任何東西**——只有設了環境變數 `AUDIT_RETENTION_DAYS`（≥30）才會由每日排程順便清理。
  - 匯出與清理本身都會留下稽核紀錄，清理另留一筆警告級錯誤日誌（事後查得到）。
- **驗證**：`npm test` 130 項全綠（新增稽核日誌 API 8 項、保留天數排程 4 項；含「設定不合理時不可刪資料」「dry-run 不可刪東西」等反向測試）；`npm run check:browser` 新增稽核日誌介面檢查 22 項（真實 Chrome 手機尺寸：分頁、篩選、實攔 Blob 檢查 CSV 內容、預覽後刪除）。

# 🏆 比賽管理系統 (Competition Manager) v3.6.5
...
### v3.6.5 (2026-09-27) - QR 掃碼報到

- **現場報到第二階段**（Roadmap 8.7⑤）：排隊更快——選手出示 QR 或把 8 碼報到碼唸給工作人員。
- 選手在「我的報名」按「📷 報到碼 / QR」→ 彈窗顯示 8 碼與 QR（只有**正取**有）。
- 管理員在「現場報到」多一列：**📷 掃碼報到 ｜ 輸入 8 碼 ｜ 簽到**；三種方式（含照舊在名單上點）都留稽核。
- **兩個誠實的限制**：
  1. **iPhone／iPad 沒有掃碼 API**（`BarcodeDetector` 只有 Android／桌面 Chrome 有）→
     「輸入 8 碼」是另一條主要路徑；沒有掃碼能力的裝置按下按鈕會**直接說明**，不開一個掃不到東西的畫面。
  2. 報到碼**不可猜**（8 碼、去掉 I／L／O／0／1、同賽事唯一、要用才產生），因為掃報名編號等於讓人幫別人簽到。
- **手寫 QR 產生器**（`public/js/qr.js`，無外部套件、CSP 不變）：byte mode＋等級 M＋版本 1–3（單一 RS 區塊）。
  **驗證方式**：結構 → 測試內另寫一份規範讀取器反向讀回 → 最後交給 **macOS Vision 真正解碼**。
  ★ 第三層抓到真 bug：格式資訊位元順序寫反（前兩層因為同一套錯誤假設而「驗過」，畫面也看不出來）。
- 測試：`npm test` **610 通過 / 0 失敗**；新增瀏覽器檢查 `checkin-qr-check.js` **28 項**（含模擬 iPhone 的降級路徑）。

### v3.6.4 (2026-09-27) - 賽事公告的一鍵推播

- **把 8.8⑥ 的推播加分項補上**：v3.6.3 讓公告「不訂閱也看得到」，這版讓「要推播的人一鍵就推得到」。
- 公告彈窗上方新增勾選框（**預設勾選**）「同時推播通知**已報名者**」，下方取消／延期／最新消息三個動作都套用。
  送出後的回覆會寫清楚結果：**「已推播給 12 位已報名者（實際送到 9 個裝置）」**。
- **只推給該賽事未刪除的報名者**：自己取消報名的人不會再收到該賽事的推播。
- 推播設定頁新增站台開關「**賽事公告通知（取消／延期／最新消息）**」；關掉後即使勾選也不送，
  且明確回報「未推播（站台設定已關閉…）」——不會讓人以為送出去了。
- **推播失敗不影響主要動作**：取消／延期本身一定成功，推播結果（送幾個裝置、失敗原因）另外記進 `push_log`。
- 測試：`npm test` **605 通過 / 0 失敗**；瀏覽器 23 支套件全綠（含推播設定 44 項）。
- 未做：QR 掃碼報到（v3.6.2 第二階段）。

### v3.6.3 (2026-09-27) - 賽事取消／延期與卡片最新消息

- **臨時變更的溝通不再依賴推播訂閱**（Roadmap 8.8⑥）：颱風延期、場地臨時不能用是常態，
  但係統的賽事狀態原本完全由日期推導——「延期過」「取消」算不出來。
- 賽事卡片 →「📣 公告／取消」（管理員以上）：
  - **⛔ 取消賽事**：必填原因（會顯示在卡片上）→ 立刻停止報名，按報名會被擋下並顯示取消原因；可「復原」。
  - **🕒 延期**：選新日期（時間可留空）→ 卡片標示「延期至 …（原訂 …）」，**原訂時間保留不覆蓋**，
    延期期間暫停報名；可「取消延期標記」。
  - **📣 最新消息**：集合時間、場地更換等公告**直接顯示在卡片**（不需訂閱推播），可清空。
- 三個動作都留稽核紀錄（`CANCEL_COMPETITION`／`POSTPONE_COMPETITION`／`UPDATE_COMPETITION_NEWS`）。
- 測試：`npm test` **605 通過 / 0 失敗**；路由覆蓋 **98／98**。
  其中 `tests/notice-render.test.js` 直接抽取 `app.js` 的真實渲染函式驗證輸出，
  **因此抓到「消息更新時間顯示 UTC 而非本地時間」的真實 bug**。
- 未做：取消時的一鍵推播公告（需先決定是否新增站台推播開關）、QR 掃碼報到。

### v3.6.2 (2026-09-27) - 現場報到與現場代報名

- **比賽當天的線下流程終於有系統可用**（Roadmap 8.7 ⑤，依你定的方式：先由管理員勾選簽到，不做 QR 掃碼）。
- 賽事卡片 →「報名／隊伍」→ 彈窗最上方是「🎫 現場報到」：**只有正取**會出現在名單上；點「簽到」即完成，
  記下**時間與操作者**；勾錯可「取消簽到」（同樣留稽核）。名單上方是「已簽到 X／應到 Y／現場代報名 Z｜未到 W」，人多可用搜尋框找姓名。
- 「➕ 現場代報名」：沒線上報名的人當場加人，標記「現場」、直接算正取；**仍守名額上限**（額滿回 409 並說明要先提高名額或處理候補）；
  同名（不分大小寫）不重複建立，改為請管理員去名單上簽到。
- 「⬇️ 匯出報名名單 (CSV)」多「簽到」「現場代報名」兩欄，可列印當紙本備援點名表。
- **資料庫**：新增 `attended_at`／`attended_by`／`onsite` 三個欄位與一個索引（可重複執行的 migration，已套用）；未跑 migration 時介面不出現、後端回 503 並指出要跑哪一支。
- **稽核**：`REGISTER_ATTENDED`／`REGISTER_ATTENDANCE_UNDONE`／`REGISTER_ONSITE`。
- 說明書同步更新（管理員操作清單第 9 步從「尚未有系統功能」改成真正的操作位置；並明文禁止說明暗示有 QR 掃碼報到）。
- 測試：`npm test` **603 通過 / 0 失敗**（新增 `attendance.test.js`）；瀏覽器檢查新增 `attendance-check.js` **35 項**（真實 Chrome 手機尺寸）。
...
- **彈窗尺寸（先量測再改）**：通知設定在 1920／1440 只有 **512px** 寬、內文 12px、按鈕高 **24px** → 桌機面板下限提高到 **704px**、內文 13px、按鈕 32px；**平板**（834 實測按鈕只有 24px，比手機的 36px 還小）比照桌機處理；**手機維持原樣**（382px／36px）。
- 作法純 CSS（`custom.css`）：桌機依原本的 `max-w-*` 分級加 `min-width` 下限（`min` 勝 `max`，不必改 HTML）；6 個漏掉 `cm-modal-panel` 的彈窗一併補上，23 個彈窗行為一致。
- 新增 `tests/browser/modal-size-check.js`（正式站四尺寸）＋`layout-check.js` 加入同樣量測，把「太細」變成可量測的數字。
- **補齊 8 條沒有測試的端點**（別名登入、修改密碼、錯誤回報、推播退訂、回收筒四條）→ 3 個新測試檔、全用假 Supabase：權限階梯、節流、欄位截斷、雜湊儲存、永久刪除真的刪掉、稽核有寫；每一項都做了 RED 驗證（故意改壞實作確認測試會轉紅）。

### v3.5.3 (2026-09-26) - 修正 v3.5.2 的一項斷言：serverless 多實例下不能用記憶體計數器

- v3.5.2 的功能**證實有效**：跑完一次完整 `check:prod`（煙霧＋訪客＋線上樣式）後，正式站錯誤日誌仍是 33 筆（修正前每次約 +30）。
- 但它加的斷言 `self_test_skipped >= 1` **本身是錯的**：那是**單一實例記憶體內的計數**，Vercel 多實例下讀 health 的請求常落到別顆實例 → 讀到 0 → 誤判失敗（96/1）。
- 改為**純行為驗證**：當場送兩個標記過的預期錯誤請求（壞 JSON＋無效權杖）→ 斷言「沒有新增 node/curl 來源日誌」且「日誌總數沒有增加（33 → 33）」；計數器降為資訊列並註明理由。
- 教訓：在 serverless／多實例環境，任何行程內計數器都不能用來證明跨請求的行為；要驗證就用遠端可對帳的事實（改動前後讀同一個總數）。
- 同時修掉另一個每天凌晨必紅的既有檢查：`ops-stats-api.test.js` 拿 **UTC** 日期比伺服器的**本地**日期，澳門 00:00–08:00 之間固定假失敗（已用 v3.5.2 worktree 重現確認與本版無關）；改用與伺服器共用的 `public/js/stats.js` 算期望值並收緊成 `===`，附 RED 驗證。
...
- 每場賽事可放一份**規程 PDF**：編輯賽事 → 「📘 賽事規程 PDF」→ 選檔 → 「上傳規程」，立刻生效（不必再儲存賽事資料）。
- 卡片出現「**📘 規程**」按鈕（帶出附件名稱），**任何人都能開**（不需登入，另開新視窗由瀏覽器開啟 PDF，可另存）；上傳／移除限管理員以上並留稽核。
- 檔案把關：**檢查檔頭 `%PDF-`**（不是看副檔名或 MIME，改檔名騙不過）、上限 **3MB**（Vercel 4.5MB 請求上限＋base64 膨脹）；擋下時會說明原因，且不會動到原有附件。
- 存法沿用海報機制（base64 存資料庫、自家端點供應）：**CSP 不必放寬、不需要外部物件儲存**；`competition_docs` 表已開啟 RLS。
- 列表只帶中介資料（標籤／大小／時間），不撈檔案內容，賽事列表不會被 3MB 的規程拖慢。
- 測試：單元 18 項＋真 Chrome 介面 32 項＋正式站煙霧 4 項。

### v3.2.1 (2026-09-26) - 版面空間優化（大螢幕不留白）

- 實測到的浪費：桌機一張 1198px 寬的卡片，左邊內容欄被右側按鈕區用 max-content 寬度擠到**只剩 84px（7%）**；日期／人數／名額一行一行往下排；主容器固定 1280px，1920 螢幕空掉 640px。
- 修正：按鈕區限寬 `min(30rem, 44%)`（固定值在平板會再擠扁，實測剩 28%，所以同時看比例）、資訊列改**自動欄數格狀**（手機 2 欄、桌機 4～7 欄）、內容欄 `min-width: 0`、大螢幕（≥1600px）容器放到 1560px。
- 結果：桌機內容欄 84→660px、卡片高 612→219px；1920 容器 1280→1560px；手機維持好讀（2 欄、13 顆按鈕全部可見、無橫向溢出）。
- 新增 `tests/browser/layout-check.js`（37 項／四種尺寸），並實測「把修正拿掉會亮紅」；線上樣式檢查加上真實版面量測（避免只是檔案改了但沒部署）。

### v3.2.0 (2026-09-26) - 名單列印版（P1-7）

- **現場要用的那張紙**：賽事卡片多了「🖨️ 列印名單」（管理員）與「🖨️ 列印成績」（已公布時所有人），
  開出 A4 列印預覽 → 用瀏覽器列印或另存 PDF。
- **名單**：紙上有賽事名稱、時間（含結束）、地點、類別、列印時間與摘要句
  （共幾人．正取幾人．候補幾人．尚餘幾個名額）；**候補自成一段並標示「候補 1、候補 2」**（依遞補順位），
  不會被誤當成要點名的人；未錄取與已取消**不印**；可切換 **包含候補／包含待審核／簽到欄／橫向**。
- **成績表**：名次照原值印（並列 1,1,3、未到場顯示「—」），狀態與成績都在。
- **列印時只印那張紙**：`@page A4`、`thead` 每頁重複、單列不跨頁，App 的按鈕與清單全部隱藏
  （用 CDP 模擬列印媒介實測過，不是只看註解）。
- **規則唯一真實來源 `public/js/printdoc.js`（UMD）**，樣式在 `public/css/print.css`
  （CSP 是 `style-src 'self'`，所以不能寫 inline style）。
- **紙本會離開系統**：列印內容只放現場必要的姓名／隊伍／成績，不含 email、密碼、雜湊或 token。
- **實測**：單元 16 項、真 Chrome 列印檢查 32 項（含 `Page.printToPDF` 真的產出 241KB PDF 且**不寫任何檔案**）。

### v3.1.2 (2026-09-26) - 修正儀表板推播區塊與刪除帳號送兩次請求

- **營運儀表板黃色警告**（`push (column push_log.created_at does not exist)`）：統計查詢讀 `push_log` 時
  select 與排序都用了 `created_at`，但這張表的時間欄位是 `sent_at`（而且那個欄位根本沒被用到——只加總計數）。
  修正後推播成功率恢復正常；測試替身同步改成與真實資料庫一致，並在單元測試與正式站煙霧加上
  「**不得有任何讀不到的區塊**」斷言（部署前實測確實紅燈，部署後轉綠）。
- **刪除帳號送出兩次請求**：`#adminList` 被綁了兩個 click 處理器（舊版遺留＋統一處理器），
  點一次會跳兩次確認、送兩個 DELETE（第二個 404）、稽核留下兩筆。移除舊監聽器＋加防連點守衛；
  順手修掉 `openAdminModal()` 每次打開都重綁「重新整理」按鈕。
- **新增真 Chrome 檢查 `tests/browser/admin-users-check.js`（20 項）**：直接數請求數與稽核筆數
  （修好前實測 2 個請求／2 次確認／2 筆稽核），另驗清單不含密碼、自己那一列沒有刪除按鈕、
  不能刪同級、一般使用者 403。
- 另外修掉一個**檢查腳本的競態**（候補名單的 modal 會先用快取繪製再被新資料取代，讀太快會讀到舊的）——
  這個紅燈在 v3.1.2 也會偶發，不是這兩版的功能造成的。
...
- **補上「賽後」那一段**：原本流程只到賽事當天，成績無處可放。現在是
  「報名 → 審核 → 賽事當天 → 登錄成績（草稿）→ 公布 → 參賽者看到名次」。
- **管理員**：賽事卡片「🏆 成績登錄」→ 名單自動帶出**已核准**的報名（未審核／已拒絕／已取消的人不會出現），
  每列填成績（自由文字：`12:34.56`、`85`、`3–2`）、狀態、名次、備註；「🔢 自動排名」一鍵算名次
  （時間類越小越好、分數類越大越好，**同分並列 1,1,3**）；「💾 儲存草稿」→「🏁 公布結果」（可同時推播通知參賽者）
  →「↩️ 取消公布」（退回草稿，成績不會被刪）。
- **參賽者**：已公布的賽事多一顆「🏆 成績」→ 前三名領獎台（🥇🥈🥉）＋完整名次表＋自己的那一列標記「（你）」
  ＋匯出 CSV；「我的報名」直接顯示 `🏆 成績：🥈 第 2 名 · 12:30`。
- **規則**：只有「完賽」且成績看得懂（時間或數字）的人進名次；看不懂的成績（`未完成`、`3–2`、`85 分`）
  **不會被當成 0 排第一名**；同分同名次是常態，所以名次刻意不設唯一鍵。
- **草稿不外洩**：未公布時對外只回「成績尚未公布」，連統計數字都不給（避免從統計反推名次）。
- **公布前檢查**（只提示不擋人）：還缺幾位參賽者的成績、名次重複、名次跳號（並列後的跳號不算異常）。
- **推播**：新增站台層開關「賽事成績公布通知」（預設開啟）；關掉只是不通知，成績照樣公布；
  只推給有報名的參賽者，並記進推播紀錄（含失敗原因與對象，可重用既有重送機制）。
- **規則唯一真實來源**：`public/js/results.js`（UMD）——後端驗證、前端選單／排名／顯示、測試共用同一份。
- **API**：`GET /api/competitions/:id/results`（公開）、`GET …/result-sheet`（管理員）、`PUT …/results`（批次儲存，
  可 `auto_rank`／`order`／`remove_ids`）、`POST …/results/publish|unpublish`（需 `confirm: true`）、`GET /api/my/results`。
- **migration**：`2026-09-27-v3.1.0-results.sql`（成績表＋三個公布欄位，全部 `IF NOT EXISTS` 可重複執行）。
  **未執行時**：成績端點回 503 並附檔名，賽事、報名、審核等既有功能完全不受影響。
- **安全**：儲存前整批驗證（名次不是正整數／報名不屬於本賽事／未核准的報名／同一筆重複送出）→ 400 且**一筆都不寫**；
  公布與取消公布都要 `confirm: true`；刪除要明確送 `remove_ids`；新表開 RLS 且不開放 anon；每個寫入都留稽核
  （`SAVE_RESULTS`／`PUBLISH_RESULTS`／`UNPUBLISH_RESULTS`）。
- **實測**：`npm test` **544/544**、`npm run check:browser` **497/497**（新增成績與結果 38 項，共 16 支檢查）。

### v3.0.0 (2026-09-26) - 後台營運儀表板與效能（統計圖表＋分頁優化＋海報縮圖）

- **營運儀表板（管理員以上）**：選單「📊 營運儀表板」一張畫面看懂現況——六個關鍵數字（賽事／報名／候補／使用者／近 30 天未處理錯誤／推播成功率）、三張手寫 SVG 圖（報名趨勢、錯誤趨勢、分類分佈，可切 7／14／30 天）、效能現況（海報縮圖省下多少、分頁上限、**實際查詢耗時**、索引數量）。
- **只回聚合數字**：沒有任何帳號、email 或單筆報名內容；一般使用者與訪客一律 403（測試會檢查回應裡不出現任何帳號名稱）。
- **降級設計**：每個區塊各自 try／catch，某張表還沒建只列在 `unavailable_sections` 並在畫面說明，不讓整個儀表板 500。結果快取 60 秒，`?fresh=1` 強制重算。
- **分頁是 opt-in**：`GET /api/competitions?limit=50&offset=100` 回 `{ items, total, has_more, paged_by }`；**沒帶 `limit` 的請求行為完全不變**（仍回陣列）。報名名單同樣支援（回應多一個 `page` 物件）。單次上限：賽事 100 筆、名單 200 筆；壞參數退回安全預設而不是報錯。`paged_by` 誠實標示是 `db`（資料庫切片）還是 `memory`（狀態／篩選要先在記憶體算）。
- **實際優化**：分頁時報名人數只撈這一頁賽事的報名（原本會掃整張表），成本從 O(全部報名) 變成 O(本頁賽事)；新增查詢次數守門測試（列表固定 3 次查詢、12 場與 100 場都一樣、禁止對同一張表重複查詢）。
- **海報縮圖**：上傳時由瀏覽器 canvas 產生縮圖（最寬 480px）一起送出，**沒有引入任何伺服器端影像套件**；`?variant=thumb` 沒有縮圖時自動退回原圖並以 `X-Poster-Variant: full-fallback` 標示；卡片用 `loading="lazy"` ＋ `decoding="async"`（視窗外的卡片完全不下載圖片，已實測）；分享／列印仍用原圖。
- **migration**：`2026-09-27-v3.0.0-perf.sql`（海報縮圖欄位＋查詢索引，全部 `IF NOT EXISTS` 可重複執行）。**未執行時**：海報上傳照常（只是不存縮圖）、`?variant=thumb` 退回原圖、儀表板照常運作並標示缺少的區塊——沒有任何功能會壞。
- **實測**：`npm test` **498/498**、`npm run check:browser` **459/459**（新增營運儀表板與列表縮圖 42 項：圖表真的畫出來、天數切換、快取行為、按鈕可見、無個資、縮圖延遲載入且捲入畫面才載入、一般使用者 403、手機無橫向溢出、無下載任何檔案）、正式站煙霧 84 項全綠。

### v2.27.1 (2026-09-26) - 把「按鈕隱形」納入自動檢查（只加檢查與文件）

- **為什麼**：使用者回報「公告中心也是這種錯誤」——公告中心的「發布公告」按鈕同樣看不到，與推播設定的「儲存設定」是**同一個根因**（`bg-sky-600` 色系在 `tailwind.min.css` 與 `custom.css` 都不存在 → 白字配透明底）。v2.27.0 已把 26 處 `sky-*` 改成 `blue-*`，但當時**只有推播設定那顆按鈕有自動檢查**。
- **新增 `tests/browser/live-style-check.js`（已納入 `npm run check:prod`）**：對正式站用真實 Chrome 算過 CSS 後檢查——四顆送出按鈕（推播設定「儲存設定」、公告中心「儲存訂閱」／「發布公告」、工作人員「指派／更新」）**有底色且文字與底色不同**；「全部標記已讀」「＋ 發布新公告」**不是白色文字**；並全頁掃描「**文字顏色＝底色**」的隱形文字。
- **公告中心介面檢查**新增 4 項按鈕可見性斷言；**工作人員介面檢查**新增 1 項。
- **實測**：線上 `live-style-check` **8 項全綠**（確認公告中心按鈕在 v2.27.0 部署後已修好；使用者截圖時間 15:02 早於 15:13 的部署）。
- **Tests**：`npm test` **453 項全綠**（未動程式邏輯）；`npm run check:browser` **417 項全綠**（公告中心 46→50、場地與工作人員 39→40）。

### v2.27.0 (2026-09-26) - 場地地圖連結＋工作人員指派（P1-5）＋修復推播設定確認按鈕

- **為什麼**：地點只有一行文字（外地選手得自己複製去地圖 App 查，舊賽事也不可能一筆筆補連結）；現場人員分工（誰是裁判、誰記錄、誰攝影）過去只能靠群組訊息。順手修掉使用者反映的「推播設定的確認按鈕沒有顯示」。
- **📍 場地地圖連結**：賽事表單新增「地圖連結 (選填)」，卡片地點旁多一個「🗺️ 地圖」連結（另開新頁、`rel=noopener`）。**沒填就用地址自動產生** Google 地圖搜尋連結並標示「（用地址搜尋）」，所以舊賽事不用補資料就有地圖可按；有填就以自訂連結為準（`www.google.com/maps/...` 這種沒帶協定的會自動補 `https://`）。只接受 `http／https`，`javascript:`／`data:`／把地址整串貼進來一律 **400 並說明原因**（不靜默丟掉）。更新時留空＝清掉自訂連結回到自動產生、完全沒帶欄位＝不動既有值；複製賽事與週期自動建立的下一場都沿用同一場地連結。規則唯一真實來源 `public/js/venue.js`（UMD 前後端共用）。
- **🧑‍⚖️ 工作人員指派**：每場賽事可指派工作人員，角色五種（**裁判／記錄／攝影／醫護／其他**）。**同一場同一人只有一個角色**（再指派＝改角色，稽核寫出「裁判 → 攝影」前後差異）；可加備註（200 字內，沒帶＝不動、帶空字串才清掉）。卡片徽章「🧑‍⚖️ 工作人員 N」＋面板顯示角色人數摘要與每列「⚖️ 裁判 王小明（第三場地）／由誰指派」。**清單公開**（只回角色與帳號名稱，不含 email／權限／token），**指派與移除只有管理員以上**且一律留稽核（`ASSIGN_STAFF`／`REMOVE_STAFF`）；停用帳號不能指派，帳號被刪除時該筆仍列出並顯示「（已刪除）」。規則唯一真實來源 `public/js/staff.js`（UMD 前後端共用）。
- **🐛 修復推播設定的「儲存設定」按鈕看不見**：按鈕用了 `bg-sky-600`，而**這個色系在 Tailwind 靜態檔與 custom.css 裡都不存在**（白字配透明底＝整顆隱形，只剩旁邊的「關閉」）。26 處 `sky-*` 改用專案既有的 `blue-*` 主按鈕色，並補上缺的 `hover:text-amber-900` 規則。**根因防護**：顏色 class 覆蓋率檢查 `scripts/class-coverage.py` 搬進 repo、色系白名單補上 `sky`（原本漏了它才沒被檢查到）、`accent-*` 納入掃描，並新增 `tests/class-coverage.test.js`，以後 `npm test` 會直接擋下不存在的顏色 class。
- **資料表探測快取修正**：以前「一發現沒有欄位／資料表就永遠記住」，導致執行完 migration 後**還在服務的暖實例仍舊回報「尚未啟用」**（線上實際發生過，只能等實例被回收）。現在負結果只快取 60 秒（正結果照舊快取），migration 跑完一分鐘內自動恢復。
- **migration**：`2026-09-26-v2.27.0-venue-staff.sql`（`competitions.map_url`＋`competition_staff`＋RLS，`IF NOT EXISTS` 可重複執行）。**未執行時**：地圖連結照常可用（改用地址自動產生）、賽事建立／更新照常但回 `map_url_saved:false`＋警告、工作人員清單回 `schema_ready:false`＋檔名、指派／移除回 503，其他功能完全不受影響。
- **Tests**：`npm test` **453 項全綠**（新增場地規則 7＋工作人員規則 12＋工作人員 API 13＋未 migration 降級 6＋覆蓋率守門 1）；`npm run check:browser` **412 項全綠**（新增「場地與工作人員介面檢查」39 項、推播設定按鈕可見性 5 項）。

### v2.26.0 (2026-09-26) - 推播失敗可視與重送＋站內公告／訊息中心

- **為什麼**：推播只留下「成功幾則」——失敗幾則、為什麼失敗、原本要送什麼給誰都看不到，訂閱過期或推播服務擋掉時，只能等使用者說「沒收到」才知道；而且沒有補救手段。Email 已確定不做，站上也缺少一個能對特定對象說一句話、又能記錄誰讀過的地方。
- **📤 推播失敗可視與重送（管理員以上）**：「📤 推播紀錄」每筆顯示成功／失敗則數、**失敗原因**（例如 `HTTP 410`）、**當時送出的內容**與**重送紀錄**；失敗的以紅框標示並提供「**重送給原本的對象**」——單人通知只送他那筆訂閱、公告只送當初名單上的人（不是全站廣播）。重送是手動指令（不受自動通知開關影響），會把重送次數／時間／成功則數寫回同一筆紀錄並留稽核 `RESEND_PUSH`（寫明送給誰）；訂閱回 `404／410` 的在重送時順手標記失效；v2.26.0 之前沒有內容的舊紀錄按重送會明確回 400 說明。
- **📣 站內公告／訊息中心（登入即可用）**：導覽選單「📣 公告中心」＋**未讀徽章**（99 以上顯示 99+），點公告即標記已讀、另有「全部標記已讀」；可勾選**訂閱的公告分類**（與賽事分類同一套 10 類）。管理員以上多一塊「管理公告」：發布／編輯、置頂、**預約發布時間**、設定結束時間、上架／下架（下架只是不顯示），以及**每則的已讀人數**（例如「已讀 3/12」）。
- **對象與可見性**：全部／管理員以上／特定分類訂閱者；發布時可勾「同時發送推播」，**只推給符合對象的使用者**（站台層「站內公告發布通知」關掉時，公告照發、推播跳過並說明原因）。管理員在訊息中心一律看得到全部上架中的公告（剛發的分類公告不該自己看不到），但**通知仍嚴格照對象送**。可見性與時間規則由前後端共用的同一份純函式 `public/js/announcements.js` 決定。
- **migration**：`2026-09-26-v2.26.0-push-log-detail.sql`（推播失敗明細與重送欄位）、`2026-09-26-v2.26.0-announcements.sql`（公告／已讀／訂閱分類＋RLS），皆 `IF NOT EXISTS` 可重複執行。**未執行時**：推播紀錄照常可看（沒有明細與重送，端點回 `detail_ready:false`＋檔名）、公告中心顯示尚未啟用並附檔名、發布／修改回 503，其他功能不受影響。
- **驗證**：`npm test` **413 項全綠**（新增公告規則 23＋公告 API 13＋公告未執行 migration 降級 5＋推播明細與重送 API 8＋推播明細降級 3）；`npm run check:browser` **368 項全綠**（新增 `announcements-check.js` **46 項**：真 Chrome 手機尺寸走完一般使用者與管理員兩條路徑，並確認沒有前端例外、沒有下載任何檔案）。
- **順手修掉**：公告管理清單「下架／重新上架」按鈕方向相反（真 Chrome 檢查抓到：按「下架」實際做了「重新上架」）。

...
- **v2.0.0** - 專案重大里程碑發佈！新增「一鍵複製比賽資訊」功能（支援格式化剪貼簿複製）；系統已完整具備全棧 CRUD、Supabase 雲端資料庫、三層權限控制 (RBAC)、動態賽事與報名狀態標籤，正式達到 Production-Ready 上線標準。
- **v1.9.0** - 新增動態報名狀態連動功能！發佈與編輯表單新增「開放報名中」勾選框，並於 Supabase 新增 `is_registration_open` 欄位；標籤可動態切換顯示「🔥 報名中」、「⏳ 即將開賽」、「⚡ 今日開賽」或「已結束」。
